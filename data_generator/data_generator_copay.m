%A = rand(3, 1e6);
tic;
fid = fopen('C:\Users\user\Dropbox\Paper\2016_journal of health economics\Cost_Sharing_Markov_Game\Cost_Sharing_Markov_Game_CP.dat', 'w+');
%fid = fopen('C:\Users\TingYu Ho\Dropbox\Paper\2016_journal of health economics\Cost_Sharing_Markov_Game\Cost_Sharing_Markov_Game_CP.dat', 'w+');

NbStates = 5 ; %normal, PH, HS1, HS2, CVD
NbAction_L = 3; %10 30 50 copayment
...
BenefitFormulary  = [0, 10 ; 0 , 30 ; 0 , 50]; %[Action_L][Action_F]
Probability  =  [0, 0.2, 0.4, 0.6, 0.8, 1]; %P_K should be the same as NbDiscreteP
...
for s=1:NbStates
    for al=1:NbAction_L
        for af=1:NbAction_F
            R_L (s,al,af) = BenefitFormulary(al,af)-CostTreatment(af)+PmtHealth_L(s);
            R_F (s,al,af) = -BenefitFormulary(al,af)+PmtHealth_F(s)+(PmtAction_F(af))^2; 
        end
    end
end
